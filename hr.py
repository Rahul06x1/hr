...
def parse_args():
    ...
    parser.add_argument('-qrd','--qrcodedimension',type=int, nargs='+',help='set custom qr code dimensions',default=[500, 500])
...
def generate_vcard(line, data, row_count):
    with open(f"vcards/{line[1].lower()}_{line[0].lower()}.vcf", "w") as f:
        f.write(data)
    logger.debug("%d Generated vCard %s", row_count, line[3])


def generate_qr_code(line, data, row_count, height, width):
    ...
    logger.debug("%d Generated qr code %s", row_count, line[3])
